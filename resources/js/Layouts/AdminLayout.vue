...
<template>
    ...
    <div class="flex grow">
        <nav id="admin-sidebar" ref="navLinks" class="flex flex-col items-center h-full overflow-hidden text-gray-700 bg-gray-100 rounded toggled">
            ...
            <div class="w-full px-2">
                <div class="flex flex-col items-center w-full mt-3">
                    <a class="flex items-center w-full h-12 px-3 mt-2 rounded nav-item" :class="route().current('pages.admin.dashboard.*') ? 'active' : ''" :href="route('pages.admin.dashboard.index')">
                    ...
                    <a class="flex items-center w-full h-12 px-3 mt-2 rounded nav-item" :class="route().current('pages.admin.course.*') ? 'active' : ''" :href="route('pages.admin.course.index')">
                        ...
                        <span class="ml-2 text-sm font-medium nav-title">Cursos</span>
                    </a>
                    <a class="flex items-center w-full h-12 px-3 mt-2 nav-item rounded" :class="route().current('pages.admin.user.*') ? 'active' : ''" :href="route('pages.admin.user.index')">
                        <svg class="w-6 h-6 stroke-current nav-icon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 512">
                            <path d="M96 128a128 128 0 1 1 256 0A128 128 0 1 1 96 128zM0 482.3C0 383.8 79.8 304 178.3 304l91.4 0C368.2 304 448 383.8 448 482.3c0 16.4-13.3 29.7-29.7 29.7L29.7 512C13.3 512 0 498.7 0 482.3zM609.3 512l-137.8 0c5.4-9.4 8.6-20.3 8.6-32l0-8c0-60.7-27.1-115.2-69.8-151.8c2.4-.1 4.7-.2 7.1-.2l61.4 0C567.8 320 640 392.2 640 481.3c0 17-13.8 30.7-30.7 30.7zM432 256c-31 0-59-12.6-79.3-32.9C372.4 196.5 384 163.6 384 128c0-26.8-6.6-52.1-18.3-74.3C384.3 40.1 407.2 32 432 32c61.9 0 112 50.1 112 112s-50.1 112-112 112z"/>
                        </svg>
                        <span class="ml-2 text-sm font-medium nav-title">Usuários</span>
                    </a>
                    <a class="flex items-center w-full h-12 px-3 mt-2 rounded nav-item" :class="route().current('pages.admin.config.*') ? 'active' : ''" :href="route('pages.admin.config.index')">
        ...
        <div class="flex w-full grow-0 flex-row">
            <nav id="admin-navbar" class="flex-row-reverse flex">
                <DropDown id="user-dropdown" :items="userDropdown" class="flex items-center h-12 pl-3 mt-2">
                    {{ userInfo.name }}
                    <img class="aspect-square flex h-8 rounded-full ml-5 object-contain" :src="userInfo.picture" @error="(event) => { event.target.src = defaultProfileImage }">
                </DropDown>
