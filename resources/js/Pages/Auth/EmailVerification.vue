...
<template>
    ...
    <div id="email-verification-page" class="flex h-screen justify-center items-center">
        <div id="container">
            ...
            <div id="form" class="w-full">
                <h1 class="text-xl font-bold">Verificação de email</h1>
                <p>Verifique o código enviado para o seu email cadastrado junto ao Marcasite Cursos e insira no campo abaixo.</p>

                <CustomInput v-model="email_code"
                    type="number"
                ...
                <a v-if="resendCode" href="#" class="mt-4 mx-auto" v-on:click="resendCodeToUser">
                    Não recebeu o código? Clique aqui e reenviaremos um novo para você!
                </a>
            </div>
        </div>
    </div>
...
export default {
    ...
    methods: {
        resendCodeToUser() {
            ...
            (async () => {
                const result = await Api.postAsync(route('api.email.send'));
        ...
        verifyCode() {
            (async () => {
                ...
                const result = await Api.postAsync(route('api.email.verify'), {email_code: this.email_code});
                if (result.code == 200) {
